...
void *proc(void *arg) {
  ...
  g->queue[0] = clCreateCommandQueue(context, dev[tid], 0, &err);
  CHECK_ERROR(err);
  g->queue[1] = clCreateCommandQueue(context, dev[tid], 0, &err);
  CHECK_ERROR(err);

  g->program = get_program("kernel.cl", context, &dev[tid], 1);
  ...
  int in = myceil(num_images, NUM_GPU);
  int is = (in / NUM_GPU) * tid;
  int ie = mymin((in / NUM_GPU) * (tid + 1), num_images);
  for(int i = is; i < ie; i += MAX_BATCH)
  {
    g->batch = mymin(ie - i, MAX_BATCH);
...
void vggnet(float *_images, float *_network, int *_labels, float *_confidences, int _num_images)
{
  ...
  err = clGetDeviceIDs(platform, CL_DEVICE_TYPE_GPU, NUM_GPU, dev, NULL);
  CHECK_ERROR(err);

  context = clCreateContext(NULL, NUM_GPU, dev, NULL, NULL, &err);
  CHECK_ERROR(err);
  ...
  for (size_t t = 0; t < NUM_GPU; ++t) {
    pthread_create(&threads[t], NULL, proc, (void*)t);
  }
  for (int t = 0; t < NUM_GPU; ++t) {
